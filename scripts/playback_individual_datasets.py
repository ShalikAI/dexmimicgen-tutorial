...
def playback_dataset(args):
    ...
    if args.filter_key is not None:
        ...
        raise RuntimeError("Could not find 'data' group in dataset")

    # Sort
    inds = np.argsort([int(elem[5:]) for elem in demos])  # assumes names like demo_00001
    demos = [demos[i] for i in inds]

    # If user provided explicit episodes, override everything
    if args.episodes is not None:
        missing = [ep for ep in args.episodes if ep not in demos]
        if missing:
            raise ValueError(f"Episodes not found in dataset: {missing}")
        demos = args.episodes

    # Otherwise maybe reduce count
...
if __name__ == "__main__":
    ...
    # Which episode to show
    parser.add_argument(
    "--episodes",
    type=str,
    nargs="+",
    default=None,
    help="Explicit episode names to playback (e.g. demo_00001 demo_00007)",
    )
